...
describe('Cabecalho', () => {
  it('mostra os quatro links vindos do JSON', () => {
    render(<Cabecalho itens={site.nav} visivel />)
  ...
  it('divide os links em dois lados, com o morcego no meio', () => {
    const { container } = render(<Cabecalho itens={site.nav} visivel />)
    ...
    expect(screen.getByRole('img', { name: /morcego/i })).toBeInTheDocument()
  })

  it('fica escondido de leitores de tela quando invisivel', () => {
    const { container } = render(<Cabecalho itens={site.nav} visivel={false} />)
